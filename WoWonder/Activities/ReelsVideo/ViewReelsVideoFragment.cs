...
using static Androidx.Media3.Container.NalUnitUtil;
    ...
    public class ViewReelsVideoFragment : AndroidX.Fragment.App.Fragment, IYouTubePlayerInitListener, StTools.IXAutoLinkOnClickListener
    {
        ...
        private void StartVideo(PostDataObject dataObject)
        {
            try
            {
                ...
                if (Methods.CheckConnectivity())
                    PollyController.RunRetryPolicyFunction(new List<Func<Task>> { () => RequestsAsync.Posts.GetPostDataAsync(dataObject?.PostId, "post_data", UserDetails.DeviceId?.Replace("-", "")) });
            }
            catch (Exception e)
            {
                Methods.DisplayReportResultTrack(e);
